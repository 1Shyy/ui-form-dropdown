...
var checkbox_select = function(params)
{
    ...
    var that            = this,
        ...
        select_name             = "ЖК,ОКРУГ,РАЙОН,МЕТРО",
        ...
        $_sel_div       = $("<div />")      .addClass("active_div"),
        $_init_div      = $("<div />")      .addClass("init_div")                   .text("s"),
        $_ul            = $("<ul />"),
        ...
        updateCurrentlySelected = function()
        {
            ...
            {
                    $_init_div.text("")
                    $_init_div.removeClass('init_div')
                    $_init_div.addClass('hide')
            }
            else if(selected.length ==  $_ul.find("input[type=checkbox]").length)
            {
                $_init_div.text( all_translation );
                $_init_div.removeClass('hide')
                $_init_div.addClass('init_div')
            }
            else
            {
                $_init_div.text( selected.length );
                $_init_div.removeClass('hide')
                $_init_div.addClass('init_div')
            }
        },
        ...
        createItem  = function(name, value, count)
        {
            ...
            return $_li.append($_label.after($_div.append($_checkbox)) );
        },
    ...
    // Build mark up before pushing into pageRoboto Condensed,
    $_dropdown_div  .append($_ul);
    $_dropdown_div  .append($_not_found);
    $_dropdown_div  .appendTo($_parent_div);
    $_sel_div  .prependTo($_parent_div);
    $_select_anchor .prependTo($_sel_div);
    $_init_div .appendTo($_sel_div);
